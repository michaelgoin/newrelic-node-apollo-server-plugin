        ...
        executionDidStart() {
          ...
          return {
            willResolveField({args, info}) {
              ...
              if (!config.captureScalars && !isTopLevelField(info) && isScalarField(info)) {
                return null
              }
...
// TODO: can prob just get rid of this and isScalarBaseType directly from invocation site.
function isScalarField(fieldInfo) {
  const result = isScalarBaseType(fieldInfo.returnType)
  return result
}

function isScalarBaseType(typeInstance) {
  const result = isScalarType(typeInstance) || isNonNullScalarType(typeInstance)
  return result
}
...
function getDetailsFromDocument(document, schema) {
  ...
  // TODO: rename
  const deepestSelectionPath = getDeepestUniqueNonScalarPath(definition, schema)
...
function getDeepestUniqueNonScalarPath(definition, schema) {
  let deepestPath = []

  const operationType =
    (definition.operation === 'query') ? schema.getQueryType() : schema.getMutationType()

  // we don't care if the first item is a scalar
  const firstSelection = getTopLevelSelection(definition, operationType)
  deepestPath.push(firstSelection.selection.name.value)

  let current = firstSelection
  while (current.selection.selectionSet) {
    current = findUniqueNonScalar(current)

    if (!current) {
      break // No unique non-scalars
    }

    // Inline fragments do not contain a name
    current.selection.name && deepestPath.push(current.selection.name.value)
  }

  return deepestPath
}

// we don't care if the top level is a scalar
function getTopLevelSelection(definition, operationType) {
  const selection = definition.selectionSet.selections[0]
  const operationFields = operationType.getFields()

  const name = selection.name.value
  const selectedField = operationFields[name]
  const selectedBaseType = getBaseType(selectedField.type)

  return { selection: selection, type: selectedBaseType}
}

function findUniqueNonScalar(parent) {
  const {selection: parentSelection, type: parentType} = parent
  const parentFields = parentType.getFields()

  let foundNonScalar = null
  for (const [, selection] of parentSelection.selectionSet.selections.entries()) {
    ...
    if (selection.kind === 'InlineFragment') {
      return { selection: selection, type: parentType }
    }

    const selectionName = selection.name.value
    const currentField = parentFields[selectionName]

    if (!currentField) {
      // We won't name further as it is invalid and we may result in MGIs.
      return null
    }

    if (!isScalarBaseType(currentField.type)) {
      if (foundNonScalar) {
        return null // Not unique. There can be only one!
      }

      const currentBaseType = getBaseType(currentField.type)
      foundNonScalar = { selection: selection, type: currentBaseType}
    }
  }
